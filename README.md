...
1. **実行環境を作成する**
   ...
   パッケージマネージャーには anaconda などを使ってもよいですが、ここでは Venv を使う方法を記載します。
...
2. **ローカルで実行する（ホットリロードあり）**
   ...
   なお、本番環境では以下のコマンドでアプリケーションを実行している。こちらは ESLint などに引っかかると起動できないので注意。
...
`--build`をつけないとコードの変更が反映されない可能性があるため、必ずつけて実行する。
